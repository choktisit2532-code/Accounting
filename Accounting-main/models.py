...
class User(Base):
    ...
    fleet_vehicles = relationship("FleetVehicle", cascade="all, delete-orphan")
...
class FleetVehicle(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "plate_number", name="uq_fleet_vehicle_user_plate"),
        CheckConstraint("vehicle_type IN ('car','motorcycle','other')", name="fleet_vehicle_type_check"),
    )
...
class FleetMileage(Base):
    __tablename__ = "fleet_mileages"
    __table_args__ = (CheckConstraint("mileage >= 0", name="fleet_mileage_nonnegative"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), index=True)
    vehicle_id: Mapped[int] = mapped_column(ForeignKey("fleet_vehicles.id", ondelete="CASCADE"), index=True)
...
class FleetExpense(Base):
    ...
    __table_args__ = (CheckConstraint("amount > 0", name="fleet_expense_positive"),)
