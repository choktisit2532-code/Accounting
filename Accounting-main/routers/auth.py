...
class LiffSessionRequest(BaseModel):
...
@router.post("/liff-session")
async def liff_session(
    payload: LiffSessionRequest,
    request: Request,
    response: Response,
    db: Session = Depends(get_db),
):
    if not settings.line_liff_id:
        raise HTTPException(status_code=503, detail="ยังไม่ได้ตั้งค่า LINE LIFF")
    rate_limiter.check(client_key(request, "liff-session"), limit=20, window_seconds=600)
    try:
        ...
        raise HTTPException(status_code=502, detail="ติดต่อ LINE ไม่สำเร็จ")
    if line_response.status_code != 200:
        raise HTTPException(status_code=401, detail="การยืนยันตัวตน LINE ไม่ถูกต้องหรือหมดอายุ")
    line_user_id = str(line_response.json().get("userId") or "")
    if not line_user_id:
        raise HTTPException(status_code=401, detail="LINE ไม่ส่งรหัสผู้ใช้กลับมา")
    user = db.query(User).filter(User.line_user_id == line_user_id).first()
    if not user:
        raise HTTPException(
            status_code=403,
            detail="บัญชี LINE นี้ยังไม่ได้เชื่อมกับ Accounting กรุณาเชื่อมบัญชีจากหน้าเว็บก่อน",
        )
    set_auth_cookie(response, create_token(user))
