...
def line_settings(secret):
    return SimpleNamespace(
        ...
        liff_fleet_url="https://example.onrender.com/fleet",
    )
...
def test_accounting_rich_menu_commands_do_not_call_gemini(client, monkeypatch):
    register_and_login(client)
    account = create_account(client)
    paired_user()
    messages = []
    ...
    async def fail_if_gemini_called(**_kwargs):
        raise AssertionError("rich-menu commands must not call Gemini")

    async def fake_reply(_token, payload):
    ...
    with SessionLocal() as db:
        user = db.query(User).filter(User.email == "user@example.com").one()
        handled = asyncio.run(line_webhook.handle_accounting_menu_command(
            db, "reply", user, "เงินอยู่ที่ไหน"
        ))

    assert handled is True
    assert messages[0]["type"] == "flex"
    assert "เงินอยู่ที่ไหน" in messages[0]["altText"]
    assert account["name"] in messages[0]["altText"]


def test_accounting_profit_menu_uses_current_month(client, monkeypatch):
    register_and_login(client)
    account = create_account(client)
    categories = client.get("/api/categories").json()
    income_category_id = next(item["id"] for item in categories if item["type"] == "income")
    expense_category_id = next(item["id"] for item in categories if item["type"] == "expense")
    user_id = paired_user()
    for tx_type, amount, category_id in (
        ("income", "2500", income_category_id),
        ("expense", "800", expense_category_id),
    ):
        assert client.post("/api/transactions", data={
            "type": tx_type, "amount": amount, "account_id": account["id"],
            "category_id": category_id, "date_val": "2026-07-20", "note": "menu test",
        }).status_code == 201
    ...
    monkeypatch.setattr(line_webhook, "settings", line_settings("line-secret"))
    monkeypatch.setattr(line_webhook, "bangkok_now", lambda: datetime(2026, 7, 20, 12, 0))
    monkeypatch.setattr(line_webhook, "reply_messages", fake_reply)
    with SessionLocal() as db:
        user = db.get(User, user_id)
        handled = asyncio.run(line_webhook.handle_accounting_menu_command(
            db, "reply", user, "สรุปกำไรเดือนนี้"
        ))

    assert handled is True
    assert "กำไรสุทธิ ฿1,700.00" in messages[0]["altText"]
