...
| `PUBLIC_BASE_URL` | URL Render จริง เช่น `https://ชื่อบริการ.onrender.com` |
| `LINE_LIFF_ID` | LIFF ID จาก LINE Developers |
| `LIFF_FLEET_URL` | URL หน้า Fleet ใน LIFF หรือเว้นว่างเพื่อใช้ `/fleet` |
...
สร้างและเปิดใช้ Rich Menu 8 ช่องหลัง Deploy:

```bash
python -m personal_finance.scripts.setup_line_rich_menu
```

เมนูประกอบด้วย เงินอยู่ที่ไหน, สรุปรายรับ, สรุปรายจ่าย, สรุปกำไร,
ถ่ายรูป, บันทึกรายการ, รถและค่าใช้จ่าย และตั้งค่า

## 5. ตรวจหลังติดตั้ง

เปิด `https://ชื่อบริการ.onrender.com/health` ต้องได้ `status: ok` และ build
`20260730.1` จากนั้นทดสอบสมัครสมาชิก, เพิ่มบัญชี, ผูก LINE, ส่งรายการ และ
ขอสรุปผ่าน LINE
...
4. ตาราง `alembic_version` ต้องมีค่า `20260729_0003`
