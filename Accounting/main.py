...
from personal_finance.routers.fleet import router as fleet_router
...
@app.get("/fleet")
def fleet_page():
    return FileResponse(static_path / "fleet.html", headers={"Cache-Control": "no-cache, no-store"})


@app.get("/api/public/line-config")
def line_public_config():
