...
CREATE TABLE public.fleet_vehicles (
    ...
    default_account_id INTEGER REFERENCES public.accounts(id) ON DELETE SET NULL,
    name VARCHAR(120) NOT NULL,
    plate_number VARCHAR(30) NOT NULL,
    vehicle_type VARCHAR(20) NOT NULL DEFAULT 'car' CHECK (vehicle_type IN ('car', 'motorcycle', 'other')),
    current_mileage INTEGER NOT NULL DEFAULT 0,
    is_active BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_fleet_vehicle_user_plate UNIQUE (user_id, plate_number)
);
CREATE TABLE public.fleet_mileages (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    vehicle_id INTEGER NOT NULL REFERENCES public.fleet_vehicles(id) ON DELETE CASCADE,
    mileage INTEGER NOT NULL CHECK (mileage >= 0),
    source VARCHAR(20) NOT NULL DEFAULT 'web',
    image_path VARCHAR(500), note TEXT,
...
CREATE TABLE public.fleet_documents (
    ...
    user_id INTEGER NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    vehicle_id INTEGER NOT NULL REFERENCES public.fleet_vehicles(id) ON DELETE CASCADE,
    document_type VARCHAR(40) NOT NULL, expiry_date DATE NOT NULL,
...
CREATE TABLE public.fleet_expenses (
    ...
    user_id INTEGER NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
    vehicle_id INTEGER NOT NULL REFERENCES public.fleet_vehicles(id) ON DELETE CASCADE,
    transaction_id INTEGER NOT NULL UNIQUE REFERENCES public.transactions(id) ON DELETE RESTRICT,
    category VARCHAR(50) NOT NULL, amount NUMERIC(14,2) NOT NULL CHECK (amount > 0),
    expense_date DATE NOT NULL, garage_name VARCHAR(150), note TEXT,
    created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX ix_fleet_vehicles_user_id ON public.fleet_vehicles (user_id);
CREATE INDEX ix_fleet_mileages_user_id ON public.fleet_mileages (user_id);
CREATE INDEX ix_fleet_mileages_vehicle_id ON public.fleet_mileages (vehicle_id);
CREATE INDEX ix_fleet_documents_user_id ON public.fleet_documents (user_id);
CREATE INDEX ix_fleet_documents_vehicle_id ON public.fleet_documents (vehicle_id);
CREATE INDEX ix_fleet_documents_expiry_date ON public.fleet_documents (expiry_date);
CREATE INDEX ix_fleet_expenses_user_id ON public.fleet_expenses (user_id);
CREATE INDEX ix_fleet_expenses_vehicle_id ON public.fleet_expenses (vehicle_id);
CREATE UNIQUE INDEX ix_fleet_expenses_transaction_id ON public.fleet_expenses (transaction_id);
CREATE INDEX ix_fleet_expenses_expense_date ON public.fleet_expenses (expense_date);
...
VALUES ('20260729_0003');

COMMIT;

-- Verification: the result should include the Accounting and Fleet tables.
SELECT tablename
FROM pg_tables
