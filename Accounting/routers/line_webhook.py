...
Example: "ค่ากาแฟ30 ค่าข้าว85 ซื้อเหล็ก580" must return 3 objects.
...
def _summary_flex(title: str, color: str, rows: list[tuple[str, str]], note: str) -> dict:
    return {
        "type": "flex",
        "altText": f"{title}: " + " · ".join(f"{label} {value}" for label, value in rows),
        "contents": {
            "type": "bubble",
            "header": {
            ...
            "footer": {
                ...
                "contents": [{"type": "button", "style": "primary", "color": color,
                              "action": {"type": "uri", "label": "เปิด Web App",
...
async def handle_accounting_menu_command(
    ...
    now = bangkok_now()
    period_note = f"ข้อมูลเดือน{THAI_MONTHS[now.month - 1]} {now.year + 543} ณ {now:%H:%M} น."

    if normalized == "เงินอยู่ที่ไหน":
        ...
        if not rows:
            rows = [("ยอดคงเหลือ", "ยังไม่มีบัญชี")]
        total = sum((Decimal(account.balance or 0) for account in accounts), Decimal("0"))
        rows.append(("รวมทุกบัญชี", _money(total)))
        await reply_messages(
            reply_token,
            [_summary_flex("เงินอยู่ที่ไหน", "#3157F6", rows, "ยอดตามบัญชีที่บันทึกไว้ในระบบ")],
        )
        return True

    command_types = {
        "สรุปรายรับเดือนนี้": ("income", "สรุปรายรับ", "#13A89E"),
        "สรุปรายจ่ายเดือนนี้": ("expense", "สรุปรายจ่าย", "#FF7A59"),
        "สรุปกำไรเดือนนี้": ("profit", "สรุปกำไร", "#7B61FF"),
    }
    ...
    if command:
        ...
        expense = Decimal(totals["expense"])
        if kind == "income":
            rows = [("รายรับรวม", _money(income)),
                    ("จำนวนรายการ", f"{totals['income_count']} รายการ")]
        elif kind == "expense":
            top = _top_expense_categories(db, user.id, start, end, limit=1)
            rows = [("รายจ่ายรวม", _money(expense)),
                    ("จำนวนรายการ", f"{totals['expense_count']} รายการ"),
                    ("หมวดสูงสุด", top[0][0] if top else "ยังไม่มีข้อมูล")]
        else:
            net = income - expense
            rows = [("รายรับ", _money(income)), ("รายจ่าย", _money(expense)),
                    ("กำไรสุทธิ", _money(net))]
        await reply_messages(reply_token, [_summary_flex(title, color, rows, period_note)])
        return True

    if normalized == "วิธีส่งรูปใบเสร็จ":
        await reply_text(
            reply_token,
            "📷 ส่งรูปสลิปหรือใบเสร็จเข้าห้องแชตนี้ได้เลย\n"
            "ระบบจะอ่านยอด วันที่ หมวด และบัญชี แล้วส่งรายการให้ตรวจสอบก่อนบันทึกทุกครั้ง",
        )
        return True
    return False
    ...
    vehicle_lines = [
        {
            ...
            "contents": [
                ...
                {"type": "text", "text": f"{vehicle.current_mileage:,} กม.", "size": "sm",
                 "color": "#667085", "align": "end", "flex": 2},
            ],
        } for vehicle in vehicles[:4]
    ] or [{"type": "text", "text": "ยังไม่มีรถในระบบ", "size": "sm", "color": "#667085"}]
    web_url = settings.liff_fleet_url or f"{settings.public_base_url}/fleet"
    return {
        "type": "flex",
        "altText": f"จัดการรถ {len(vehicles)} คัน · ค่าใช้จ่ายเดือนนี้ ฿{Decimal(expense or 0):,.2f}",
        "contents": {
            "type": "bubble",
            "header": {
                ...
                "paddingAll": "20px",
                "contents": [
                    {"type": "text", "text": "SMART FINANCE · FLEET", "size": "xs", "color": "#C9D5FF", "weight": "bold"},
                    {"type": "text", "text": "รถของฉัน", "size": "xl", "color": "#FFFFFF", "weight": "bold", "margin": "sm"},
                ],
            },
            "body": {
                ...
                "contents": [
                    {"type": "box", "layout": "horizontal", "contents": [
                        {"type": "text", "text": "ค่าใช้จ่ายเดือนนี้", "size": "sm", "color": "#667085"},
                        {"type": "text", "text": f"฿{Decimal(expense or 0):,.2f}", "weight": "bold", "align": "end"},
                    ]},
                    {"type": "box", "layout": "horizontal", "contents": [
                        {"type": "text", "text": "เอกสารใกล้หมดอายุ", "size": "sm", "color": "#667085"},
                        {"type": "text", "text": f"{due} รายการ", "weight": "bold", "align": "end", "color": "#E94C67" if due else "#12A594"},
                    ]},
            ...
            "footer": {
                ...
                "contents": [{"type": "button", "style": "primary", "color": "#FF7A59",
                              "action": {"type": "uri", "label": "เปิดระบบจัดการรถ", "uri": web_url}}],
            },
        },
    }
...
async def handle_fleet_command(db: Session, reply_token: str, user: User, text: str) -> bool:
    normalized = re.sub(r"\s+", "", text.lower())
    if normalized not in {"จัดการรถ", "รถของฉัน", "fleet", "สรุปรถ"}:
        return False
    ...
    if text is not None:
        parts.append({
            "text": (
                ...
                "Resolve Thai relative dates such as วันนี้ and เมื่อวาน from this date. "
        ...
        with SessionLocal() as db:
            ...
            try:
                ...
                if msg_type == "text" and len(analyses) == 1:
                    ...
                    messages.append({
                        "type": "text",
                        "text": f"พบ {len(analyses)} รายการ กรุณาตรวจสอบและยืนยันทีละรายการ",
                    })
