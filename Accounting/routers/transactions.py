...
def update_transaction(
    ...
    if tx.source == "fleet":
        raise HTTPException(status_code=409, detail="กรุณาแก้รายการนี้จากหน้า “จัดการรถ” เพื่อให้ข้อมูลตรงกัน")
    proposed = {
...
def delete_transaction(tx_id: int, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if not tx:
        raise HTTPException(status_code=404, detail="ไม่พบรายการนี้")
    if tx.source == "fleet":
        raise HTTPException(status_code=409, detail="กรุณาลบรายการนี้จากหน้า “จัดการรถ” เพื่อคืนยอดอย่างถูกต้อง")
