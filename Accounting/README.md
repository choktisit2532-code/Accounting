...
- โมดูล Fleet ใช้สมาชิกและฐานข้อมูลเดียวกับ Accounting
- ค่าใช้จ่ายรถเชื่อมรายการรายจ่าย Accounting แบบหนึ่งต่อหนึ่ง
- LINE Flex Message, Rich Menu และ LIFF เปิด Fleet ภายใน LINE โดยไม่ล็อกอินซ้ำ
...
| PUBLIC_BASE_URL | เมื่อใช้ LINE/LIFF | URL จริงของระบบ เช่น https://example.onrender.com |
| LINE_LIFF_ID | เมื่อใช้ LIFF | LIFF ID จาก LINE Developers |
| LIFF_FLEET_URL | ไม่บังคับ | URL LIFF เต็ม หากเว้นว่างใช้ PUBLIC_BASE_URL/fleet |
...
1. สร้าง LIFF App แบบ Full และตั้ง Endpoint URL เป็น `https://YOUR-DOMAIN/fleet`
2. ใส่ LIFF ID ใน `LINE_LIFF_ID`
3. ใส่ URL LIFF เต็มใน `LIFF_FLEET_URL` หรือปล่อยว่างเพื่อใช้หน้า `/fleet`
4. หลัง Deploy รัน `python -m personal_finance.scripts.setup_line_rich_menu` หนึ่งครั้ง

Rich Menu มี 8 ปุ่ม: เงินอยู่ที่ไหน, สรุปรายรับ, สรุปรายจ่าย, สรุปกำไร,
ถ่ายรูป, บันทึกรายการ, รถและค่าใช้จ่าย และตั้งค่า คำสั่งสรุปจาก Rich Menu
อ่านข้อมูลของบัญชีที่ผูก LINE โดยตรงและไม่ส่งคำสั่งเหล่านี้ไปให้ Gemini

เมื่อเปิด Fleet จาก LINE ระบบตรวจ access token กับ LINE API และสร้าง Session Cookie
ของ Accounting ให้เฉพาะ LINE User ID ที่ผูกบัญชีแล้วเท่านั้น
